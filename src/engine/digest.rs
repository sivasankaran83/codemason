...
/// One diagnostic code and the occurrences grouped under it: file, line and
/// message. Named because it was written out twice and clippy was right
/// that the second spelling is where it drifts.
type GroupedByCode<'a> = (&'a String, &'a Vec<(String, usize, String)>);
...
pub fn digest_tsc(text: &str) -> String {
    ...
    if total > 0 {
        ...
        let mut codes: Vec<GroupedByCode<'_>> = by_code.iter().collect();
        codes.sort_by_key(|(_, items)| std::cmp::Reverse(items.len()));
