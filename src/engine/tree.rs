...
use crate::engine::graph::{DependencyGraph, Symbol};
...
fn render_node(
    ...
    for (i, (name, child)) in entries.iter().enumerate() {
        ...
        // A directory, or a file that turned out to hold children, both read
        // as a directory to a person looking at the tree.
        ...
        if opts.symbols
            && child.is_file
            && let Some(path) = &child.file_path
            && let Some(syms) = top_level_symbols(graph, path)
        {
            out.push_str("  (");
            out.push_str(&syms);
            out.push(')');
