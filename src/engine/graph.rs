...
pub struct FileNode {
    ...
    #[serde(skip)]
    pub source: String,
    /// Needed because c# resolves through a project wide symbol table rather
    /// than per import, so the resolver has to know which files are c#.
    #[serde(skip)]
    pub language: String,
}
...
impl DependencyGraph {
    ...
    pub fn add_file(&mut self, file_path: &str, source: &str, language: &str) {
        ...
        let (symbols, raw_imports, package_name) = if language == csharp::LANGUAGE {
            (
                csharp::extract_symbols(source, &root),
                csharp::extract_imports(source, &root),
                csharp::extract_namespace(source, &root),
            )
        } else {
            (
                extract_symbols(source, language, &root),
                extract_imports(source, language, &root),
                extract_package(source, language, &root),
            )
        };
    ...
    pub fn resolve_dependencies(&mut self) {
        ...
        let csharp_resolver = csharp::Resolver::build(&self.files);

        let resolutions: Vec<(String, Vec<String>)> = self
            .files
            .iter()
            .map(|(fp, node)| {
                if node.language == csharp::LANGUAGE {
                    return (fp.clone(), csharp_resolver.resolve(fp, node));
                }
                let deps = node
                    .raw_imports
                    .iter()
        ...
        for (name, locations) in &defined {
            ...
            if !referenced && locations.len() == 1 {
                ...
                if let Some(node) = self.files.get(fp)
                    && symbol_used_in_source(&node.source, name, sym.line)
                {
                    continue;
                }
                let dep_count = self.reverse.get(fp).map(|v| v.len()).unwrap_or(0);
            ...
            "type_declaration" => {
                ...
                for child in node.children(&mut c) {
                    if child.kind() == "type_spec"
                        && let Some(name) = find_child_text(source, &child, "name")
                    {
                        return Some(Symbol {
                            name,
                            kind: "type".to_string(),
        ...
        match language {
            ...
            "javascript" | "typescript" => {
                if child.kind() == "import_statement"
                    && let Some(src) = child.child_by_field_name("source")
                {
            ...
            "kotlin" => {
                if child.kind() == "import"
                    && let Some(name) = find_child_by_kind(source, &child, "qualified_identifier")
            ...
            "ruby" => {
                ...
                if child.kind() == "call"
                    && let Some(path) = extract_ruby_require(source, &child)
                {
                    imports.push(path);
                }
            }
        ...
        if child.kind() == "import_spec_list" {
            ...
            for spec in child.children(&mut inner_cursor) {
                if spec.kind() == "import_spec"
                    && let Some(path) = spec.child_by_field_name("path")
                {
                    let text = node_text(source, &path);
...
fn resolve_import(
    ...
    // Rust: crate::module::item -> module. The prefix is every crate's own
    // root, so it is matched as written and never narrowed to a module name
    // from this workspace.
